  ...
  if (type === "object") {
    ...
    if (schema.additionalProperties === true) {
      ...
      if (!isAnySchema(schema.additionalProperties)) {
        ...
        // Don't mark parent as unsupported just because nested additionalProperties are complex
        // The parent object renderer can still show the map editor
    ...
    if (!itemsSchema) {
      ...
      // Don't mark array as unsupported just because items are complex objects
      // The array renderer can still show add/remove UI with nested editors
      // if (res.unsupportedPaths.length > 0) unsupported.add(pathLabel);
    }
  } else if (
    type !== "string" &&
